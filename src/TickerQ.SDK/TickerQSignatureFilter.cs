...
public sealed class TickerQSignatureFilter : IEndpointFilter
{
    private const long MaxTimestampSkewSeconds = 300;

    private readonly TickerSdkOptions _options;
    private readonly ILogger<TickerQSignatureFilter>? _logger;

    public TickerQSignatureFilter(TickerSdkOptions options, ILogger<TickerQSignatureFilter>? logger = null)
    {
        ...
        EndpointFilterDelegate next)
    {
        ...
        // Validate WebhookSignature is configured
        if (string.IsNullOrWhiteSpace(_options.WebhookSignature))
        {
            _logger?.LogWarning("TickerQ signature validation skipped: WebhookSignature not configured for {Method} {Path}",
        ...
        {
            _logger?.LogWarning("TickerQ signature validation failed: Missing X-TickerQ-Signature header for {Method} {Path}",
                request.Method, request.Path);
            return Results.Unauthorized();
        }
        ...
        {
            _logger?.LogWarning("TickerQ signature validation failed: Missing X-Timestamp header for {Method} {Path}",
                request.Method, request.Path);
            return Results.Unauthorized();
        }
        ...
        {
            _logger?.LogWarning("TickerQ signature validation failed: Invalid timestamp format for {Method} {Path}",
                request.Method, request.Path);
            return Results.Unauthorized();
        }
        ...
        {
            _logger?.LogWarning("TickerQ signature validation failed: Timestamp skew too large ({SkewSeconds}s) for {Method} {Path}",
        ...
        try
        {
            received = Convert.FromBase64String(sig!);
        }
        catch (FormatException)
        {
            _logger?.LogWarning("TickerQ signature validation failed: Invalid Base64 signature for {Method} {Path}",
                request.Method, request.Path);
            return Results.Unauthorized();
        }

        // Enable buffering before reading so downstream can read again
        ...
        // Compute expected signature (empty body is valid)
        ...
        {
            _logger?.LogWarning("TickerQ signature validation failed: Signature mismatch for {Method} {Path}",
                request.Method, request.Path);
            return Results.Unauthorized();
        }
